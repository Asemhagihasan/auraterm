# copilot.md — Project Instructions

Copilot MUST import and use /data/services.ts directly as the data source.
Copilot must NOT ask for confirmation or alternative data sources.
All pages and components must read from this file.

**Project:** Industrial Services Website
**Stack:** Next.js (App Router) + Tailwind CSS + shadcn/ui + motion.dev
**Goals:** Fully responsive, SEO-first, structured data, accessible, animated UI.
...
- Animations: **motion.dev (NOT framer-motion)**
- Images: **next/image**
- Fonts: **next/font**
...
## 2) Single Source of Truth (Data Contract)

All services must be generated from the data below.
...
and export the following array.
...
Use the service slug to load data from `/data/services.ts`.

---

## 5) Mandatory Section Order on Each Service Page

1. Hero (serviceTitle + shortDescription)
2. What is this service (longDescription)
3. Methods (mainMethods)
...
Title and description must come from:
...
Each service page MUST inject the following JSON-LD:
...
## 8) Performance & SEO Coding Rules
...
- All images must have meaningful `alt` generated from `imageIdeas`

---

## 9) motion.dev Animation Rules
...
- Use `inView` animations
- Stagger children transitions for service grids
- Respect `prefers-reduced-motion`

---

## 10) shadcn/ui Components to Use
...
- Motion reduced automatically when user prefers reduced motion

---

## 14) General Instruction

All UI, pages and animations must be generated only from the
`services` dataset defined in `/data/services.ts`.

This dataset is the only source of truth.
